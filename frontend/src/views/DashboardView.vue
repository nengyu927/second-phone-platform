...
const emptySummary=()=>({memberCount:0,productCount:0,orderCount:0,repairCount:0,tradeInCount:0,pendingTradeInCount:0,pendingOrderCount:0,activeRepairCount:0,completedRepairCount:0,lowStockProductCount:0,totalOrderAmount:0,completedOrderAmount:0})
...
const mainCards=computed(()=>[
  {title:'會員總數',value:summary.value.memberCount,icon:'👥',to:'/admin/members'},
  {title:'商品總數',value:summary.value.productCount,icon:'📱',to:'/admin/products'},
  {title:'訂單總數',value:summary.value.orderCount,icon:'📦',to:'/admin/orders'},
  {title:'維修單總數',value:summary.value.repairCount,icon:'🔧',to:'/admin/repairs'},
  {title:'收購申請',value:summary.value.tradeInCount,icon:'↺',to:'/admin/trade-ins'}
])
const statusCards=computed(()=>[
  ...
  ,{title:'待檢測收購',value:summary.value.pendingTradeInCount,label:'待處理',warning:summary.value.pendingTradeInCount>0}
])
...
<template><section class="dashboard-page">
  ...
  <section><h3>快速操作</h3><div class="quick-actions"><RouterLink class="primary" to="/admin/members">會員管理</RouterLink><RouterLink class="primary" to="/admin/products">新增商品</RouterLink><RouterLink class="primary" to="/admin/orders">處理訂單</RouterLink><RouterLink class="primary" to="/admin/repairs">維修管理</RouterLink><RouterLink class="primary" to="/admin/trade-ins">收購管理</RouterLink></div></section>

  <section><h3>最近資料</h3><div class="recent-grid">
    <article class="card recent-card"><header><h4>最近會員</h4><RouterLink to="/admin/members">查看全部</RouterLink></header><p v-if="!recentData.recentMembers.length">尚無會員資料</p><ul v-else><li v-for="m in recentData.recentMembers" :key="m.id"><div><strong>#{{m.id}} {{m.account||m.name}}</strong><span>{{m.email||m.phone||'-'}}</span></div><time>{{formatDate(m.createdAt)}}</time></li></ul></article>
    <article class="card recent-card"><header><h4>最近商品</h4><RouterLink to="/admin/products">查看全部</RouterLink></header><p v-if="!recentData.recentProducts.length">尚無商品資料</p><ul v-else><li v-for="p in recentData.recentProducts" :key="p.id"><div><strong>#{{p.id}} {{p.productName}}</strong><span>{{formatCurrency(p.price)}}｜庫存 {{p.stock??0}}</span></div><time>{{formatDate(p.createdAt)}}</time></li></ul></article>
    <article class="card recent-card"><header><h4>最近訂單</h4><RouterLink to="/admin/orders">查看全部</RouterLink></header><p v-if="!recentData.recentOrders.length">尚無訂單資料</p><ul v-else><li v-for="o in recentData.recentOrders" :key="o.id"><div><strong>#{{o.id}}｜會員 {{o.memberId}}</strong><span>{{formatCurrency(o.totalAmount)}}｜{{orderLabels[o.orderStatus]||o.orderStatus}}</span></div><time>{{formatDate(o.createdAt)}}</time></li></ul></article>
    <article class="card recent-card"><header><h4>最近維修</h4><RouterLink to="/admin/repairs">查看全部</RouterLink></header><p v-if="!recentData.recentRepairs.length">尚無維修資料</p><ul v-else><li v-for="r in recentData.recentRepairs" :key="r.id"><div><strong>#{{r.id}}｜會員 {{r.memberId}}</strong><span>{{r.deviceBrand}} {{r.deviceModel}}｜{{repairLabels[r.repairStatus]||r.repairStatus}}</span></div><time>{{formatDate(r.receivedAt||r.createdAt)}}</time></li></ul></article>
  </div></section>
</section></template>
