...
<template>
  <section class="management-page">
    <div class="page-heading"><div><h2>商品管理</h2><p>管理二手手機商品與庫存。</p></div></div>
    ...
    <form class="card" @submit.prevent="submitForm">
      ...
      <div class="form-grid">
        ...
        <label>狀態<select v-model="form.status"><option>AVAILABLE</option><option>SOLD_OUT</option><option>DISABLED</option></select></label>
    ...
    <form class="card search-bar" @submit.prevent="loadProducts">
      ...
      <label>狀態<select v-model="filters.status"><option value="">全部</option><option>AVAILABLE</option><option>SOLD_OUT</option><option>DISABLED</option></select></label>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="product in products" :key="product.id">
              ...
              <td>{{ formatPrice(product.price) }}</td><td>{{ product.stock }}</td><td><span class="badge">{{ product.status }}</span></td>
